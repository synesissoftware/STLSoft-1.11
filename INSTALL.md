...
1. Download the latest distribution, e.g. [**STLSoft-1.11-1.11.1-beta7.zip**](https://github.com/synesissoftware/STLSoft-1.11/releases/tag/1.11.1-beta7);
...
$ mkdir -p ~/open-source
$ cd ~/open-source
$ unzip STLSoft-1.11-1.11.1-beta7.zip
```

3. Define an environment variable `STLSOFT`, whose value is the directory in which you unzipped it, e.g. `STLSOFT=~/open-source/STLSoft-1.11-1.11.1-beta7`, and then specify `$(STLSOFT)/include` (**UNIX**) or `%STLSOFT%\include` (**Windows**) in your project files and makefiles;
